...
def main():
    ...
    if start_monitoring:
        ...
        # 检查2025年的特殊情况
        if 2025 in config['years']:
            st.warning("""
            ⚠️ **关于2025年论文的说明**
            
            2025年的学术会议论文可能还未发布或仍在审核中：
            - **ICLR 2025**: 通常在4-5月发布最终论文
            - **CVPR 2025**: 通常在3-4月发布最终论文  
            - **NeurIPS 2025**: 通常在9-10月发布最终论文
            
            建议：
            1. 先尝试爬取2023-2024年的论文
            2. 或者等待2025年论文正式发布
            3. 可以关注会议的submission deadline和review timeline
            """)
            
            # 让用户选择是否继续
            continue_2025 = st.checkbox("我了解风险，继续尝试爬取2025年数据")
            if not continue_2025:
                st.info("请修改年份选择后重新开始监控")
                return
        ...
        try:
            ...
            if not papers_data:
                ...
                可能的原因：
                1. **网络连接问题** - 请检查网络连接
                2. **会议年份问题** - 2025年论文可能还未发布
                3. **网站结构变化** - 目标网站可能更新了结构
                4. **访问限制** - 网站可能有反爬虫机制
                ...
                # 提供快速修复建议
                st.info("💡 **快速修复建议**: 尝试选择 ICLR 2024 + 关键词 'transformer' 进行测试")
                return
